...
@click.command()
@click.option("--input-style", default="old", show_default=True, help="old (block*.dat) or new (HDF5) style for input files")
@click.option('--block-pattern',  default="block([0-9]+).dat", show_default=True, help="regex to match a block number in blockX.dat")

@click.option('--extra-pattern',   multiple=True, default=["SMC([0-9]+).dat"], show_default=True, help="regex pattern to match 'extra' files with file number in parentheses. This argument can be repeated")
@click.option('--extra-pattern-name',  multiple=True, default=["lef_positions"], show_default=True, help="key under which to store (can be repeated)" )
@click.option('--extra-loader',   multiple=True, default=["pickle.load(open(filename,'rb'))"], show_default=True, help="python expression f(filename) that loads data (can be repeated)")
@click.option('--extra-require/--extra-not-require',   multiple=True, default=[False], show_default=True, help="Require or not the current extra to be present (can be repeated)")

@click.option('--overwrite/--not-overwrite', default=False, show_default=True, help="raise error if files exist in destination")
@click.option('--allow-nonconsecutive', is_flag=True, help="allow blocks to be non-consecutive (1,2,3...)")
...
@click.option('--HDF5-blocks-per-file', default=50 , show_default=True,  help="blocks per file for HDF5 reporter")
@click.option('--max-unmatched-files', default=20 , show_default=True,  help="maximum number of extra files found")
@click.option("--replace", is_flag=True, help="use out_dir as temp dir, and replace in_dir with out_dir")
@click.option("--force-delete", is_flag=True, help="delete subdirectories")


@click.argument('IN_DIR')
@click.argument('OUT_DIR')
def trajcopy(block_pattern, 
             extra_pattern, 
             extra_pattern_name, 
             extra_loader,
             extra_require,
