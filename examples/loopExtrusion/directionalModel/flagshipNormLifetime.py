...
def initModel(i=0):
    ...
    SMCTran = smcTranslocatorDirectional(birthArray, myDeathArray, stallArrayLeft, stallArrayRight, pauseArray, stallDeathArray, smcNum)
    return SMCTran
...
def doPolymerSimulation(steps, dens, stiff, folder ):
    ...
    while True:
        ...
        a.setup(platform="CUDA", PBC=True, PBCbox=[box, box, box], GPU=sys.argv[4], precision="mixed")
        a.saveFolder(folder)
...
doPolymerSimulation(steps=5000, stiff=2, dens=0.2, folder="flagshipLifetime{1}Mu{2}_try={0}".format(sys.argv[1], sys.argv[2], sys.argv[3]))
